...
ENV CONFIG "\
    ...
    -DENABLE_DTRACE=OFF \
    -DMYSQL_SERVER_SUFFIX='-EricDatabase-' \
    -DWITH_UNIT_TESTS=0 \
    -DWITHOUT_TOKUDB=ON \
    -DWITHOUT_ROCKSDB=ON \
    -DWITH_PAM=ON \
...
RUN addgroup -S mysql && adduser -D -S -h /var/cache/mysql -s /sbin/nologin -G mysql mysql \
    ...
    && mkdir -p /usr/src \
    && add perl perl-dbi perl-term-readkey && perl -MCPAN -e 'install DBD::mysql' \
    ...
    && apk del .build-deps  && chmod +x /usr/local/bin/docker-entrypoint.sh
