...
RUN groupadd mysql && useradd -r -g mysql -s /bin/false mysql \
    && mkdir -p $DATA_DIR $LOGS_DIR  ${ETC_DIR} && yum install -y \
        cmake gcc g++ make bison kernel-devel openssl-devel openssl libxml2-devel gcc-c++ which lsof holland-xtrabackup socat nc boost-program-options ncurses-devel --skip-broken \
